    ...
    public func toNSCollectionLayoutGroup() -> NSCollectionLayoutGroup {
        let group = if repeatItemsCount > 0 {
            if let firstItem = toNSCollectionLayoutItems().first {
                NSCollectionLayoutGroup.horizontal(layoutSize: size, subitem: firstItem, count: repeatItemsCount)
            } else {
                NSCollectionLayoutGroup.horizontal(layoutSize: size, subitems: toNSCollectionLayoutItems())
            }
        } else {
            NSCollectionLayoutGroup.horizontal(layoutSize: size, subitems: toNSCollectionLayoutItems())
        }
